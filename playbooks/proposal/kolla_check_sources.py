...
def apply_update(name, info, new_v, new_hashes, sources_path):
    ...
    Loads sources.yaml with a round-trip YAML parser, mutates only the
    affected fields in memory, and writes it back out. This preserves
    comments and formatting for everything else in the file.
    """
...
def check_versions(sources, sources_path, repo_root, update=False):
    ...
    :param sources: the parsed kolla/common/sources.yaml data
